...
class ControlAnonTest2(GhostTestFramework):
    ...
    def run_test(self):
        ...
        # 1 - Create a transaction inside node 0 which has no blacklists and submit it to node 1 which has blacklists
        # 2 - Try to spend from blacklisted txs inside node 1 and it will be rejected
        ...
        try:
            ...
            assert_equal(self.wait_for_mempool(nodes[0], tx), False)

            # raw_blinded_tx_node1 = nodes[1].getrawtransaction(tx)
            # res = nodes[1].sendrawtransaction(raw_blinded_tx_node1)
