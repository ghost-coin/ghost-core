...
void AvailableCoins(const CWallet& wallet, std::vector<COutput>& vCoins, const CCoinControl* coinControl, std::optional<CFeeRate> feerate, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t nMaximumCount)
{
    ...
    if (wallet.IsParticlWallet()) {
        ...
        return phdw->AvailableCoins(vCoins, coinControl, feerate, nMinimumAmount, nMaximumAmount, nMinimumSumAmount, nMaximumCount);
    }
        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (wallet.IsParticlWallet()) {
        ...
        bool rv = phdw->CreateTransaction(vecSend, tx, nFeeRet, nChangePosInOut, error, coin_control, fee_calc_out, sign);
        if (!rv) {
            error = _("CreateTransaction failed.");
            return std::nullopt;
        }
