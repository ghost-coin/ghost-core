...
static RPCHelpMan pushtreasuryfundsetting()
{
    ...
    ghost::TreasuryFundSettings settings(setting["fundaddress"].get_str(), setting["minstakepercent"].getInt<int>(), setting["outputperiod"].getInt<int>());
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                ...
                auto init = ipc->spawnProcess("ghost-node");
                echo = init->makeEcho();
