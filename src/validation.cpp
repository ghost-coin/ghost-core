...
MempoolAcceptResult ChainstateManager::ProcessTransaction(const CTransactionRef& tx, bool test_accept, bool ignore_locks)
{
    ...
    auto result = AcceptToMemoryPool(*active_chainstate.m_mempool, active_chainstate, tx, GetTime(), /* bypass_limits= */ false, test_accept, ignore_locks);
        ...
        while (num--) {
            ...
            if (nTime > nNow - nExpiryTimeout) {
                ...
                if (AcceptToMemoryPool(pool, active_chainstate, tx, nTime, /* bypass_limits= */ false,
                                               /* test_accept= */ false, /* ignore_locks= */ false).m_result_type == MempoolAcceptResult::ResultType::VALID) {
                    ++count;
                } else {
